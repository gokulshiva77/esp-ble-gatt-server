...
static char device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "Gokul_BLE_Device";
...
{
    ESP_LOGI(GATTS_TAG, "%s Event - %d", __func__, event);
    switch (event) {
...
void example_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param){
    ESP_LOGI(GATTS_TAG, "%s, event %d, gatts_if %d, conn_id %d)", __func__, param->write.event, gatts_if, param->write.conn_id);
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ESP_LOGI(GATTS_TAG, "%s, event %d, gatts_if %d", __func__, event, gatts_if);
    switch (event) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        strncpy((char *)rsp.attr_value.value, "Hello", 6);
    ...
    case ESP_GATTS_ADD_CHAR_EVT: {
        ...
        ESP_LOGI(GATTS_TAG, "Get Register Char Length = %x", length);
        for(int i = 0; i < length; i++){
            ESP_LOGI(GATTS_TAG, "Get Register Char Data[%x] =%x",i,prf_char[i]);
        }
